...
public class PSet11 {
	...
	private static int[][] blockOptions(int[][] inputBlocks, int num) {
		...
		for(int i = 0; i < len; i++){
	    		...
	    		currBlock = inputBlocks;
	    	
	        blockArray[3*i][0] = Math.min(currBlock[i][0], currBlock[i][1]);
	        blockArray[3*i][1] = Math.max(currBlock[i][0], currBlock[i][1]);
	        blockArray[3*i][2] = (currBlock[i][2]);
	        
	        blockArray[3*i + 1][0] = Math.min(currBlock[i][0], currBlock[i][2]);
	        blockArray[3*i + 1][1] = Math.max(currBlock[i][0], currBlock[i][2]);
	        blockArray[3*i + 1][2] = currBlock[i][0];
	        
	        blockArray[3*i + 2][0] = Math.min(currBlock[i][1], currBlock[i][2]);
	        blockArray[3*i + 2][1] = Math.max(currBlock[i][1], currBlock[i][2]);
	        blockArray[3*i + 2][2] = currBlock[i][0];
	        
	       // blockArray[3*i + 1][] = {Math.min(currBlock[i][0], currBlock[i][2]), Math.max(currBlock[i][0], currBlock[i][2]), currBlock[i][1]};
	       // blockArray[3*i + 2][] = {Math.min(currBlock[i][1], currBlock[i][2]), Math.max(currBlock[i][1], currBlock[i][2]), currBlock[i][0]};
